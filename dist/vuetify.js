...
(function (global, factory) {
  ...
  const VDataTableHeaders = defineComponent({
    ...
    setup(props, _ref) {
      return () => {
        return props.rows.map((row, i) => vue.createVNode("tr", {
    ...
    setup(props, _ref) {
      ...
      return () => {
        if (props.showLoader && props.loading) {
          return createRange(props.stopIndex - props.startIndex).map(i => vue.createVNode("tr", {
            "class": "v-data-table__tr",
            "role": "row",
            "key": `loading_${i}`
          }, [props.columns.map(column => vue.createVNode("td", {
        ...
        return props.items.map((item, rowIndex) => item[VDataTableExpandedKey] ? vue.createVNode("tr", {
        ...
        }, [vue.createTextVNode("expanded row")], 4)]) : vue.createVNode("tr", {
          "class": "v-data-table__tr",
          "role": "row",
          "onClick": () => {
  ...
  const useHeaders = props => {
    ...
    const tableGridStyles = vue.computed(() => ({
  ...
  const createExpanded = props => {
    const expanded = vue.ref(new Map());
    ...
    vue.provide(VDataTableExpandedKey, {
      toggleExpand
    });
    return {
      items,
  ...
  const VDataTable = defineComponent({
    ...
    setup(props, _ref) {
      ...
      }, null, 8, ["rows", "rowHeight", "sticky"])]), vue.createVNode("tbody", {
        "class": "v-data-table__tbody",
        "role": "rowgroup"
      }, [vue.createVNode(VDataTableRows, {
  ...
  const useVirtual = (props, itemsLength) => {
    ...
    const totalHeight = vue.computed(() => itemsLength.value * itemHeight.value);
    const scrollTop = vue.ref(0); // TODO: Scroll threshold should be relative to current position to avoid fixed load positions

    const chunkSize = vue.ref(30); // TODO: Should reflect height of container

    const windowSize = vue.computed(() => itemHeight.value * (chunkSize.value * 3)); // const scrollIndex = computed(() => Math.floor(scrollTop.value / itemHeight.value))
    ...
    const stopIndex = vue.computed(() => Math.min(startIndex.value + chunkSize.value * 3, itemsLength.value));
    const offsetStart = vue.computed(() => Math.max(0, startIndex.value * itemHeight.value));
    const isScrolling = vue.ref(false);
    const startOffset = vue.computed(() => itemHeight.value * startIndex.value);
    let scrollTimeout = null;
    vue.watch(startIndex, val => {
      vm.emit('load', {
  ...
  const VVirtualDataTable = defineComponent({
    ...
    setup(props, _ref) {
      ...
      const {
        ...
        isScrolling,
        itemHeight
      } = useVirtual(props, vue.computed(() => {
        var _props$itemsLength;

        return (_props$itemsLength = props.itemsLength) != null ? _props$itemsLength : expandedItems.value.length;
      }));
      const items = vue.computed(() => props.itemsLength ? expandedItems.value : expandedItems.value.slice(startIndex.value, stopIndex.value));
      vue.watch(isScrolling, val => loading.value = val);
      return () => {
        return vue.createVNode("div", {
          "ref": containerRef,
